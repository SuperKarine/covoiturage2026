...
# Dépendances (développement)
    ...
    # Installe MongoDB, Composer, et outils pour Bootstrap
    RUN apt-get update && apt-get install -y \
        unzip curl git wget libcurl4-openssl-dev pkg-config libssl-dev \
    ...
    # Installe les dépendances PHP (production uniquement)
    ...
    # Installation extensions PHP + outils nécessaires
    RUN apt-get update && apt-get install -y \
        unzip curl git wget libcurl4-openssl-dev pkg-config libssl-dev \
     && pecl install mongodb-1.21.0 \
     && docker-php-ext-enable mongodb \
    ...
    # Installation de Composer (utile si besoin de commands en prod)
    ...
    # Copie des fichiers Bootstrap depuis le stage de développement
