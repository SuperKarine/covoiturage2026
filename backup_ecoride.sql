/*M!999999\- enable the sandbox mode */ 
-- MariaDB dump 10.19  Distrib 10.11.13-MariaDB, for debian-linux-gnu (aarch64)
--
-- Host: localhost    Database: ecoride2026
-- ------------------------------------------------------
-- Server version	10.11.13-MariaDB-ubu2204

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;

--
-- Table structure for table `Compte`
--

DROP TABLE IF EXISTS `Compte`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Compte` (
  `id_compte` int(11) NOT NULL AUTO_INCREMENT,
  `date_creation` datetime NOT NULL,
  `solde` float NOT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  PRIMARY KEY (`id_compte`),
  KEY `id_utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `Compte_ibfk_1` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Compte`
--

LOCK TABLES `Compte` WRITE;
/*!40000 ALTER TABLE `Compte` DISABLE KEYS */;
INSERT INTO `Compte` VALUES
(1,'2026-04-20 09:00:00',0,1),
(2,'2026-04-20 09:10:10',25.5,2),
(3,'2026-04-20 09:20:06',12,3),
(4,'2026-04-20 10:03:02',40.75,4),
(5,'2026-04-20 11:04:14',8.2,5),
(6,'2026-04-20 10:17:53',0,6),
(7,'2026-04-20 10:18:53',0,7);
/*!40000 ALTER TABLE `Compte` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `DistanceVille`
--

DROP TABLE IF EXISTS `DistanceVille`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `DistanceVille` (
  `id_distance_ville` int(11) NOT NULL AUTO_INCREMENT,
  `id_ville_depart` int(11) NOT NULL,
  `id_ville_arrivee` int(11) NOT NULL,
  `km` float NOT NULL,
  PRIMARY KEY (`id_distance_ville`),
  KEY `id_ville_depart` (`id_ville_depart`),
  KEY `id_ville_arrivee` (`id_ville_arrivee`),
  CONSTRAINT `DistanceVille_ibfk_1` FOREIGN KEY (`id_ville_depart`) REFERENCES `Ville` (`id_ville`),
  CONSTRAINT `DistanceVille_ibfk_2` FOREIGN KEY (`id_ville_arrivee`) REFERENCES `Ville` (`id_ville`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `DistanceVille`
--

LOCK TABLES `DistanceVille` WRITE;
/*!40000 ALTER TABLE `DistanceVille` DISABLE KEYS */;
INSERT INTO `DistanceVille` VALUES
(1,1,2,140),
(2,2,4,200),
(3,1,3,110),
(4,1,4,75),
(5,3,6,35),
(6,1,5,52),
(7,5,2,61),
(8,11,1,40),
(9,12,1,35),
(10,2,7,80),
(11,2,8,60),
(12,8,9,55),
(13,1,10,52),
(14,10,11,33);
/*!40000 ALTER TABLE `DistanceVille` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `MessagerieMail`
--

DROP TABLE IF EXISTS `MessagerieMail`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `MessagerieMail` (
  `id_messagerie_mail` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('inscription','reset','compte','Autre') NOT NULL,
  `Sujet` varchar(255) NOT NULL,
  `contenu` text NOT NULL,
  `date_envoi` datetime NOT NULL,
  `statut` enum('envoye','Brouillon','reçu') NOT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  `id_reservation` int(11) DEFAULT NULL,
  PRIMARY KEY (`id_messagerie_mail`),
  KEY `id_reservation` (`id_reservation`),
  KEY `id_utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `MessagerieMail_ibfk_1` FOREIGN KEY (`id_reservation`) REFERENCES `Reservation` (`id_reservation`),
  CONSTRAINT `MessagerieMail_ibfk_2` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Messagerie mail pour inscription et validation reservation';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `MessagerieMail`
--

LOCK TABLES `MessagerieMail` WRITE;
/*!40000 ALTER TABLE `MessagerieMail` DISABLE KEYS */;
INSERT INTO `MessagerieMail` VALUES
(1,'inscription','Inscription','Bienvenue sur Covoiturage2026,\r\nVotre inscription est confirmée. Vous pouvez dès maintenant réserver ou proposer des trajets.','2026-04-20 10:15:18','envoye',1,NULL),
(2,'inscription','Inscription','Confirmation de création de compte,\r\nMerci pour votre inscription. Votre compte utilisateur est actif.','2026-04-20 10:20:40','envoye',2,NULL),
(3,'reset','Réinitialisation mot de passe','Cliquez sur le lien pour réinitialiser votre mot de passe. Le lien expire dans 30 minutes.\r\n ','2026-04-21 09:00:00','envoye',3,NULL),
(4,'compte','Modification de votre profil','Votre profil utilisateur a bien été mis à jour.\r\n ','2026-04-21 12:30:53','envoye',4,NULL),
(5,'Autre','Bienvenue dans l’équipe covoiturage','Nous sommes heureux de vous compter parmi nos utilisateurs actifs.\r\n','2026-04-21 18:00:00','envoye',5,NULL),
(6,'compte','Vérification de votre compte','Merci de vérifier votre adresse email pour activer toutes les fonctionnalités.','2026-04-22 08:45:39','envoye',1,NULL),
(7,'inscription','Compte validé avec succès','Votre compte a été validé. Bon voyage sur Covoiturage2026 !','2026-04-22 14:23:52','envoye',2,7),
(8,'compte','Réservation confirmée','Votre réservation de trajet Lille → Amiens a bien été confirmée.','2026-04-22 14:26:12','envoye',3,8),
(9,'compte','Annulation de réservation','Votre réservation pour le trajet Amiens → Calais a été annulée.\r\n','2026-04-22 16:27:22','envoye',3,9),
(10,'Autre','Rappel de trajet','N’oubliez pas votre trajet prévu demain matin. Soyez à l’heure au point de rendez-vous.','2026-04-23 07:29:00','envoye',4,10);
/*!40000 ALTER TABLE `MessagerieMail` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Notes`
--

DROP TABLE IF EXISTS `Notes`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Notes` (
  `id_note` int(11) NOT NULL,
  `note` int(11) DEFAULT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  `id_trajet` int(11) NOT NULL,
  `id_auteur` int(11) NOT NULL,
  KEY `id_utilisateurs` (`id_utilisateurs`),
  KEY `id_trajet` (`id_trajet`),
  KEY `id_auteur` (`id_auteur`),
  CONSTRAINT `Notes_ibfk_1` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `Notes_ibfk_2` FOREIGN KEY (`id_trajet`) REFERENCES `Trajets` (`id_trajet`),
  CONSTRAINT `Notes_ibfk_3` FOREIGN KEY (`id_auteur`) REFERENCES `Utilisateurs` (`id_utilisateurs`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='passager donne note au chauffeur';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Notes`
--

LOCK TABLES `Notes` WRITE;
/*!40000 ALTER TABLE `Notes` DISABLE KEYS */;
INSERT INTO `Notes` VALUES
(1,5,6,1,2),
(2,4,6,1,3),
(3,5,7,2,4),
(4,3,7,2,5),
(5,4,6,3,2),
(6,5,6,3,4),
(7,4,7,4,3);
/*!40000 ALTER TABLE `Notes` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Reservation`
--

DROP TABLE IF EXISTS `Reservation`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Reservation` (
  `id_reservation` int(11) NOT NULL AUTO_INCREMENT,
  `status` enum('en attente','valide','annulée') NOT NULL,
  `date_reservation` datetime NOT NULL,
  `id_trajet` int(11) NOT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  PRIMARY KEY (`id_reservation`),
  KEY `id_trajet` (`id_trajet`),
  KEY `id_utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `Reservation_ibfk_1` FOREIGN KEY (`id_trajet`) REFERENCES `Trajets` (`id_trajet`),
  CONSTRAINT `Reservation_ibfk_2` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='réserver un trajet';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Reservation`
--

LOCK TABLES `Reservation` WRITE;
/*!40000 ALTER TABLE `Reservation` DISABLE KEYS */;
INSERT INTO `Reservation` VALUES
(7,'valide','2026-04-20 10:00:36',1,2),
(8,'en attente','2026-04-20 10:05:33',2,3),
(9,'valide','2026-04-20 10:10:21',3,4),
(10,'annulée','2026-04-20 10:15:02',4,5),
(11,'valide','2026-04-20 10:20:48',5,2);
/*!40000 ALTER TABLE `Reservation` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Role`
--

DROP TABLE IF EXISTS `Role`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Role` (
  `id_role` int(11) NOT NULL AUTO_INCREMENT,
  `name` varchar(250) NOT NULL,
  PRIMARY KEY (`id_role`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Role`
--

LOCK TABLES `Role` WRITE;
/*!40000 ALTER TABLE `Role` DISABLE KEYS */;
INSERT INTO `Role` VALUES
(1,'Admin'),
(2,'passager'),
(3,'chauffeur');
/*!40000 ALTER TABLE `Role` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Trajets`
--

DROP TABLE IF EXISTS `Trajets`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Trajets` (
  `id_trajet` int(11) NOT NULL AUTO_INCREMENT,
  `nbr_places_dispo` int(11) NOT NULL,
  `fumeur` tinyint(1) NOT NULL,
  `animaux` tinyint(1) NOT NULL,
  `prix` float NOT NULL,
  `date_depart` datetime NOT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  `id_ville_depart` int(11) NOT NULL,
  `id_ville_arrivee` int(11) NOT NULL,
  `id_voiture` int(11) NOT NULL,
  PRIMARY KEY (`id_trajet`),
  KEY `id_utilisateurs` (`id_utilisateurs`),
  KEY `id_ville_depart` (`id_ville_depart`),
  KEY `id_ville_arrivee` (`id_ville_arrivee`),
  KEY `id_voiture` (`id_voiture`),
  CONSTRAINT `Trajets_ibfk_1` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `Trajets_ibfk_2` FOREIGN KEY (`id_ville_depart`) REFERENCES `Ville` (`id_ville`),
  CONSTRAINT `Trajets_ibfk_3` FOREIGN KEY (`id_ville_arrivee`) REFERENCES `Ville` (`id_ville`),
  CONSTRAINT `Trajets_ibfk_4` FOREIGN KEY (`id_voiture`) REFERENCES `Voitures` (`id_voiture`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Trajets`
--

LOCK TABLES `Trajets` WRITE;
/*!40000 ALTER TABLE `Trajets` DISABLE KEYS */;
INSERT INTO `Trajets` VALUES
(1,3,0,1,15,'2026-04-25 08:00:08',6,1,2,1),
(2,2,0,0,18.5,'2026-04-25 09:30:23',7,2,3,2),
(3,4,1,1,10,'2026-04-26 07:46:42',6,1,4,1),
(4,3,0,1,10,'2026-04-26 16:49:58',7,4,1,2),
(5,2,0,0,17,'2026-04-27 10:51:30',6,3,2,1);
/*!40000 ALTER TABLE `Trajets` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Transactions`
--

DROP TABLE IF EXISTS `Transactions`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Transactions` (
  `id_transactions` int(11) NOT NULL,
  `date` datetime NOT NULL,
  `statut` enum('validé','en attente','annulée') NOT NULL,
  `type` enum('debit','credit') NOT NULL,
  `montant` float NOT NULL,
  `id_compte_source` int(11) NOT NULL,
  `id_compte_destination` int(11) NOT NULL,
  `id_reservation` int(11) NOT NULL,
  KEY `id_compte_source` (`id_compte_source`),
  KEY `id_compte_destination` (`id_compte_destination`),
  KEY `id_reservation` (`id_reservation`),
  CONSTRAINT `Transactions_ibfk_1` FOREIGN KEY (`id_compte_source`) REFERENCES `Compte` (`id_compte`),
  CONSTRAINT `Transactions_ibfk_2` FOREIGN KEY (`id_compte_destination`) REFERENCES `Compte` (`id_compte`),
  CONSTRAINT `Transactions_ibfk_3` FOREIGN KEY (`id_reservation`) REFERENCES `Reservation` (`id_reservation`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='pour débiter un passager et créditer un chauffeur par rappor';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Transactions`
--

LOCK TABLES `Transactions` WRITE;
/*!40000 ALTER TABLE `Transactions` DISABLE KEYS */;
INSERT INTO `Transactions` VALUES
(1,'2026-04-20 10:05:17','validé','debit',15,1,2,7),
(2,'2026-04-20 11:46:25','validé','credit',15,2,1,7),
(3,'2026-04-20 10:07:24','en attente','debit',15,3,2,8),
(4,'2026-04-20 10:49:33','validé','debit',18.5,2,3,8),
(5,'2026-04-20 10:50:43','validé','credit',18.5,4,3,9),
(6,'2026-04-20 11:52:05','annulée','debit',18.5,3,4,9),
(7,'2026-04-20 10:53:16','validé','debit',10,5,4,10),
(8,'2026-04-20 10:55:01','validé','credit',10,4,5,10),
(9,'2026-04-20 17:56:02','en attente','debit',10,2,5,11);
/*!40000 ALTER TABLE `Transactions` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Utilisateurs`
--

DROP TABLE IF EXISTS `Utilisateurs`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Utilisateurs` (
  `id_utilisateurs` int(11) NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `prenom` varchar(255) NOT NULL,
  `username` varchar(50) DEFAULT NULL,
  `mail` varchar(250) NOT NULL,
  `password` varchar(250) NOT NULL,
  `confirmation_token` varchar(100) DEFAULT NULL,
  `tel` varchar(20) NOT NULL,
  `id_compte` int(11) NOT NULL,
  `id_role` int(11) NOT NULL,
  PRIMARY KEY (`id_utilisateurs`),
  UNIQUE KEY `mail` (`mail`),
  UNIQUE KEY `username` (`username`),
  KEY `id_compte` (`id_compte`),
  KEY `id_role` (`id_role`),
  CONSTRAINT `Utilisateurs_ibfk_1` FOREIGN KEY (`id_compte`) REFERENCES `Compte` (`id_compte`),
  CONSTRAINT `Utilisateurs_ibfk_2` FOREIGN KEY (`id_role`) REFERENCES `Role` (`id_role`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Table des utilisateurs';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Utilisateurs`
--

LOCK TABLES `Utilisateurs` WRITE;
/*!40000 ALTER TABLE `Utilisateurs` DISABLE KEYS */;
INSERT INTO `Utilisateurs` VALUES
(1,'Admin','Systeme','sysad','admin@covoiturage2026.fr','$2y$10$abcdefghijklmnopqrstuv',NULL,'600000001',1,1),
(2,'Dupont','Jean','jeadu','jean.dupont@mail.com','$2y$10$abcdefghijklmnopqrstuv',NULL,'600000002',2,2),
(3,'Martin','Claire','claima','claire.martin@mail.com','$2y$10$abcdefghijklmnopqrstuv',NULL,'600000003',3,2),
(4,'Bernard','Lucas','luber','lucas.bernard@mail.com','$2y$10$abcdefghijklmnopqrstuv',NULL,'600000004',4,2),
(5,'Petit','Sophie','sopet','sophie.petit@mail.com','$2y$10$abcdefghijklmnopqrstuv',NULL,'600000005',5,2),
(6,'Durand','Marc','madur','marc.durand@mail.com','$2y$10$hash',NULL,'600000006',6,3),
(7,'Moreau','Julie','jumo','julie.moreau@mail.com','$2y$10$hash',NULL,'600000007',7,3);
/*!40000 ALTER TABLE `Utilisateurs` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Ville`
--

DROP TABLE IF EXISTS `Ville`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Ville` (
  `id_ville` int(11) NOT NULL AUTO_INCREMENT,
  `nom` varchar(250) NOT NULL,
  PRIMARY KEY (`id_ville`)
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Ville`
--

LOCK TABLES `Ville` WRITE;
/*!40000 ALTER TABLE `Ville` DISABLE KEYS */;
INSERT INTO `Ville` VALUES
(1,'Lille'),
(2,'Amiens'),
(3,'Calais'),
(4,'Dunkerque'),
(5,'Arras'),
(6,'Boulogne-sur-mer'),
(7,'Saint-Quentin'),
(8,'Beauvais'),
(9,'Compiègne'),
(10,'Valenciennes'),
(11,'Douai'),
(12,'Lens');
/*!40000 ALTER TABLE `Ville` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `Voitures`
--

DROP TABLE IF EXISTS `Voitures`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `Voitures` (
  `id_voiture` int(11) NOT NULL AUTO_INCREMENT,
  `marque` varchar(255) NOT NULL,
  `model` varchar(255) NOT NULL,
  `nbr_places` int(11) NOT NULL,
  `id_utilisateurs` int(11) NOT NULL,
  PRIMARY KEY (`id_voiture`),
  KEY `id_utilisateurs` (`id_utilisateurs`),
  CONSTRAINT `Voitures_ibfk_1` FOREIGN KEY (`id_utilisateurs`) REFERENCES `Utilisateurs` (`id_utilisateurs`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Les voitures des utilisateurs ';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `Voitures`
--

LOCK TABLES `Voitures` WRITE;
/*!40000 ALTER TABLE `Voitures` DISABLE KEYS */;
INSERT INTO `Voitures` VALUES
(1,'Dacia','Sandero',4,6),
(2,'peugeot','308',5,7),
(3,'Toyota','Yaris',4,6),
(4,'Volkswagen','Golf',5,7);
/*!40000 ALTER TABLE `Voitures` ENABLE KEYS */;
UNLOCK TABLES;
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;

/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;

-- Dump completed on 2026-05-12 14:50:51
